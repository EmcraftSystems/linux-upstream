 ...
 * PLL divisors table:
 * Fref: MCLK input
 * Fs: sampling rate, which is obtained from Fout:
 * Fout: Fs x 4 x P x 256 (e.g. 44100 x 4 x 1 x 256), where
 *       numbers 4 and 256 are just contants, P is the master clock prescaller,
 *       which is set in nau8822_hw_params() depending on <sampling rate>
 * Ndiv = 0 => R = Fout / Fref = N.abcdefgh
 * Ndiv = 1 => R = Fout / (Fref / 2) = N.abcdefgh
 * where:
 * - N is the integer part from the range [6; 12];
 * - if N is out of the [6; 12] range, then Ndiv is chaged so that
 *   the resulted N could fit into [6; 12]
 * - Ki: are the fractional parts:
 *   K = (2^24) * 0.abcdefgh, and convert to neares 24-bit value
 *   K1: high order bits (6 bits)
 *   K2: middle order bits (9 bits)
 *   K3: low order bits (9 bits)
 * So, if you have some different fref or need to support some special
 * <fs>, then just add the appropriate items into the table below.
 * Example:
 * Fref = 12MHz
 * Fout = 44.1KHz * 4 * 2 * 256 = 90.3168MHz
 * Let's try without MCLK/2, i.e. Ndiv=0, then:
 * R = Fout/Fref = 90.3168 / 12 = 7.5264 => N=7 fits in [6; 12] range,
 * So use:
 * - Ndiv = 0;
 * - N = 7;
 * - K = (2^24) * 0.1584 = 8831526.5024 ~= 8831526 = 0x86C226 =>
 *   K1 = 0x021 ((0x86C226 >> 18) & 0x03F)
 *   K2 = 0x161 ((0x86C226 >>  9) & 0x1FF)
 *   K3 = 0x026 ((0x86C226 >>  0) & 0x1FF)
 */
static const struct pll_div nau8822_pll_div[] = {
	/* fs = 44.1KHz and harmonics */
	{ 12000000, 44100, 0x021, 0x161, 0x026, 0x07},    /* MCLK=12Mhz */
	{ 14400000, 44100, 0x011, 0x0D0, 0x1CA, 0x06},    /* MCLK=14.4Mhz */
	{ 19200000, 44100, 0x01A, 0x039, 0x0B0, 0x09},    /* MCLK=19.2Mhz */
	{ 19800000, 44100, 0x007, 0x1BB, 0x0F8, 0x09},    /* MCLK=19.8Mhz */
	{ 24000000, 44100, 0x021, 0x161, 0x026, 0x07},    /* MCLK=24Mhz */
	{ 26000000, 44100, 0x03C, 0x145, 0x1D4, 0x06},    /* MCLK=26Mhz */
	/* fs = 48KHz and harmonics */
	{ 12000000, 48000, 0x00C, 0x093, 0x0E9, 0x08},    /* MCLK=12Mhz */
	{ 14400000, 48000, 0x034, 0x1D0, 0x06D, 0x06},    /* MCLK=14.4Mhz */
	{ 19200000, 48000, 0x00F, 0x0B8, 0x0A3, 0x0A},    /* MCLK=19.2Mhz */
	{ 19800000, 48000, 0x03B, 0x100, 0x09E, 0x09},    /* MCLK=19.8Mhz */
	{ 24000000, 48000, 0x00C, 0x093, 0x0E9, 0x08},    /* MCLK=24Mhz */
	{ 26000000, 48000, 0x023, 0x1EA, 0x126, 0x07},    /* MCLK=26Mhz */
};
...
err:
	...
	dev_err(codec_dai->dev, "fref = %d, fs = %d\n", fref, fs);
	return -EINVAL;
}
...
#else
# define nau8822_suspend	NULL
# define nau8822_resume		NULL
#endif
...
static int nau8822_read_device_properties(struct device *dev,
					  struct nau8822_priv *nau8822)
{
	nau8822->mclk = devm_clk_get(dev, "mclk");
				      ...
				      const struct i2c_device_id *id)
{
	...
	if (!nau8822) {
		nau8822 = devm_kzalloc(dev, sizeof(struct nau8822_priv),
				      GFP_KERNEL);
		if (!nau8822)
			return -ENOMEM;
	...
	ret = nau8822_read_device_properties(dev, nau8822);
	if (ret)
		return ret;
...
static const struct of_device_id nau8822_of_ids[] = {
	{ .compatible = "nuvoton,nau8822", },
	{}
};
MODULE_DEVICE_TABLE(of, nau8822_of_ids);

static struct i2c_driver nau8822_i2c_driver = {
	.driver = {
		.name = "nau8822",
		.of_match_table = of_match_ptr(nau8822_of_ids),
		.owner = THIS_MODULE,
	},
