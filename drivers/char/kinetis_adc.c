...
static long adc_enable_module(unsigned int mod, int enable)
{
	...
	if (mod >= N_ADC_MODULES || !adc_modules[mod].regs ||
		!adc_modules[mod].clk) {
		rv = -ENODEV;
		goto out;
	}
	...
	if (enable) {
		clk_prepare_enable(adc_modules[mod].clk);
		...
		clk_disable(adc_modules[mod].clk);
	}
...
static int __init kinetis_adc_init(void)
{
	...
	np = of_find_node_by_name(NULL, "adc");
	if (!np) {
		pr_err("%s: can't find ADC desc in device tree", adc_dev.name);
		return -ENODEV;
	}
	...
	for (i = 0; i < N_ADC_MODULES; i++) {
		...
		adc_modules[i].regs = of_io_request_and_map(np, i, adc_dev.name);
		adc_modules[i].clk = of_clk_get(np, i);
	}
...
static void __exit kinetis_adc_exit(void)
{
	...
	for (i = 0; i < N_ADC_MODULES; i++) {
		...
		if (adc_modules[i].clk)
			clk_put(adc_modules[i].clk);
	}
}
