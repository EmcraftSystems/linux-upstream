...
#define DRIVER_NAME		"stm32f4-usart"
...
static int stm32_startup(struct uart_port *port)
{
	...
	if (stm32_port->prepare_rx) {
		ret = stm32_port->prepare_rx(port, USART_DR,
					     STM32_SERIAL_RINGSIZE);
		if (ret < 0)
			stm32_set_ops(port);
	}
	if (stm32_port->prepare_tx) {
		ret = stm32_port->prepare_tx(port, USART_DR);
		if (ret < 0)
			stm32_set_ops(port);
	}
...
MODULE_DESCRIPTION("STMicroelectronics STM32F4 serial port driver");
