	     ...
	     unsigned long __user *rc, void __user *frame)
{
	...
	if (current->personality & FDPIC_FUNCPTRS) {
		...
		funcptr = (void *)ksig->ka.sa.sa_handler;
	...
	if (ksig->ka.sa.sa_flags & SA_RESTORER) {
		if (current->personality & FDPIC_FUNCPTRS) {
			/*
			 * for fdpic we ensure that restorer is call with a
			 * correct r9 value for that we need to write code on
			 * stack that setup r9 and jump back to restorer value
			 */
			struct fdpic_func_descriptor __user *funcptr;
			funcptr = (void *)ksig->ka.sa.sa_restorer;
			if (thumb) {
			...
			/*
			 * last word of rc is data and so we don't need to invalidate
			 * icache for it
			 */
